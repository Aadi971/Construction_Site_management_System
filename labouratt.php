...
if ($id && $siteid) {
    $query = "SELECT 
                labour_attendence.*,
                labour_page.lname AS employee_name, 
                site.site_name
              FROM labour_attendence
              JOIN site ON labour_attendence.site_id = site.id
              JOIN labour_page ON labour_attendence.l_id = labour_page.id
              WHERE labour_attendence.site_id = $siteid AND labour_attendence.l_id = $id
              LIMIT 1";
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        $att_sql = "SELECT curr_date, attendence FROM labour_attendence 
                    WHERE l_id = '$id' AND site_id = '$siteid' 
